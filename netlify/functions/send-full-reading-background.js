// ============================================================
// send-full-reading-background.js — Netlify Background Function
// Generates a rich 6-section natal birth chart reading and
...
exports.handler = async function (event) {
  ...
  const profileRes = await fetch(
    `${SUPABASE_URL}/rest/v1/profiles?id=eq.${userId}&select=name,email,birth_date,birth_time,birth_city,subscribed,pro_expires_at`,
  ...
  if (!profile) {
    console.error(`[send-full-reading] User not found: ${userId}`);
    return { statusCode: 404, body: JSON.stringify({ error: 'User not found' }) };
  }

  const today = new Date().toISOString().slice(0, 10);
  const hasActiveTrial = profile.pro_expires_at && profile.pro_expires_at >= today;
  if (!profile.subscribed && !hasActiveTrial) {
    console.error(`[send-full-reading] Non-Pro user attempted full reading: ${userId}`);
  ...
  if (!name || !email || !birthDate || !birthCity) {
    console.error(`[send-full-reading] Incomplete profile for ${userId}`);
    return { statusCode: 400, body: JSON.stringify({ error: 'Incomplete profile — birth data required' }) };
  }

  try {
    // ── 1. Calculate chart ────────────────────────────────────
    const chart = await calculateChart(birthDate, birthTime || null, birthCity);
...
${placements}`;
    ...
    console.log(`[send-full-reading] Starting generation for ${name} (${email})`);
...
Write A WORD TO CARRY — a personal, poetic closing of 2 paragraphs that speaks directly to ${name} as a whole person, integrating everything the chart reveals. This is not a summary — it's a final word that lands with weight. Speak to the central tension or invitation of this chart: the thing this particular combination of placements seems to be asking them to do, become, or integrate in this lifetime. Make it feel like something worth saving. 2 rich, unhurried paragraphs.`, 900);
    ...
    console.log(`[send-full-reading] All sections generated for ${name}. Sending email...`);

    // ── 3. Build and send email ───────────────────────────────
    await sendChartEmail({ name, email, birthDate, birthTime, birthCity, chart, sections: { s1, s2, s3, s4, s5, s6 } });

    console.log(`[send-full-reading] Email sent to ${email}`);
    return { statusCode: 200, body: JSON.stringify({ success: true }) };

  } catch (err) {
    console.error('[send-full-reading] Error:', err.message);
